...
import { Star, Users, Calendar, Clock, X, CheckCircle, Info } from 'lucide-react';
...
export default function MentoringPage() {
    ...
    const handleBooking = () => {
        if (selectedSlots.length === 0) return alert('Pilih minimal satu sesi jadwal!');
        ...
        // Sort slots chronologically (simple string sort works for "Day, HH:mm" if day order is standard, but here we just list them)
            ...
            {selectedTutor && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-md rounded-[2rem] p-6 shadow-2xl space-y-6 animate-in zoom-in-95 relative overflow-hidden">
                        ...
                        {/* Decorative Header */}
                        <div className={`absolute top-0 left-0 right-0 h-24 ${selectedTutor.color} opacity-30`}></div>

                        {/* Avatar & Close */}
                        <div className="relative z-10 flex justify-between items-start">
                            <div className="mt-4">
                                <div className={`w-16 h-16 rounded-full bg-white p-1 shadow-sm mb-3`}>
                                    <div className={`w-full h-full rounded-full ${selectedTutor.color} flex items-center justify-center text-slate-700 font-bold text-2xl`}>
                                        {selectedTutor.name.charAt(0)}
                                    </div>
                                </div>
                                <h3 className="font-bold text-slate-900 text-xl">{selectedTutor.name}</h3>
                                <p className="text-sm text-slate-500">{selectedTutor.skill}</p>
                            </div>
                            <button onClick={() => setSelectedTutor(null)} className="bg-white p-2 rounded-full hover:bg-slate-100 shadow-sm">
                        ...
                        <div>
                            ...
                            <div className="flex p-1 bg-slate-100 rounded-xl">
                                <button
                                    ...
                                    className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${bookingType === 'private' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                                >
                                    Private
                                </button>
                                <button
                                    onClick={() => setBookingType('group')}
                                    className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${bookingType === 'group' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                                >
                                    Kelompok (Patungan)
                                </button>
                            </div>
                        ...
                        <div>
                            <label className="text-sm font-bold text-slate-700 mb-2 block flex items-center gap-2 justify-between">
                                ...
                                <span className="text-xs font-normal text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">Bisa pilih {'>'}1</span>
                            </label>
                                        ...
                                        for (let h = startHour; h < endHour; h++) {
                                            ...
                                            slots.push(
                                                <button
                                                    ...
                                                    className={`px-3 py-3 rounded-xl text-xs font-bold border transition-all text-center flex items-center justify-center gap-2
                                                        ${isSelected
                        ...
                        <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                            ...
                            <button
                                ...
                                className="bg-yellow-400 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-yellow-500 text-slate-900 font-bold px-8 py-3 rounded-xl shadow-lg transition-all active:scale-95 flex items-center gap-2"
                            >
                                Booking
                            </button>
                        </div>
